"""
Quantifies relevance scores for specific facial regions (e.g., eyes, nose, mouth) using MediaPipe landmarks on LRP heatmaps from the VGG-Hum model.
...
heatmaps_dir = local_paths.DIR_AVERAGE_HEATMAPS_HUM
results_dir = local_paths.DIR_REGION_ANALYSIS_RESULTS
csv_path = os.path.join(results_dir, "landmark_annotations.csv")
output_csv = os.path.join(results_dir, "relevance_per_region_dataset_hj.csv")
...
for filename in tqdm(sorted(os.listdir(images_dir))):
    ...
    heatmap_path = os.path.join(heatmaps_dir, filename.replace("-", "_").replace("_frontal.png", "_average_odd_one_out_VGGFaceHumanJudgment.npy"))
