"""
Trains an Autoencoder on the LRP heatmaps from the VGG-MaxP model.
Learns a compressed latent representation of the relevance maps to compare the structural organization with the human-aligned model.
"""
...
dataset = HeatmapDataset(local_paths.DIR_SINGLE_MaxP_HEATMAPS, transform)
...
np.save(os.path.join(output_dir,"latent_codes_MaxP.npy"), latents)
np.save(os.path.join(output_dir,"image_paths_MaxP.npy"), all_paths)


# --- Save training log to CSV ---
log_filename = os.path.join(output_dir,"train_log_AE_MaxP.csv")
...
# --- Save model weights ---
model_save_path = os.path.join(output_dir,"AE_weights_MaxP.pth")
...
plt.tight_layout() 
plt.savefig(os.path.join(output_dir,"reconstruction_AE_MaxP.png"))
