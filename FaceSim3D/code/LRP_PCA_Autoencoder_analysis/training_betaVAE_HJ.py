"""
Trains a Beta-VAE on the LRP heatmaps from the VGG-Hum model.
Investigates if disentangled latent representations capture interpretable features of facial relevance for the structural analysis.
"""
...
beta = 4.0   # try 1, 4, 10 to see effects
...
for epoch in range(epochs):
    ...
    for imgs, _ in tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
        ...
        loss, recon_l, kl_l = beta_vae_loss(recon, imgs, mu, logvar, beta)
        loss.backward()
        optimizer.step()
    ...
    print(f"Epoch {epoch+1}: total={total_loss/len(dataloader):.4f}, "
          f"recon={total_recon/len(dataloader):.4f}, kl={total_kl/len(dataloader):.4f}")

# --- Extract Latent Codes ---
print("Extracting latent representations...")
model.eval()
all_latents, all_paths = [], []
with torch.no_grad():
...
np.save(os.path.join(output_dir,"latent_codes_beta_VAE_HJ.npy"), latents)
np.save(os.path.join(output_dir,"image_paths_beta_VAE_HJ.npy"), all_paths)


# --- Save training log to CSV ---
log_filename = os.path.join(output_dir,"train_log_beta_VAE_HJ.csv")
...
# --- Save model weights ---
model_save_path = os.path.join(output_dir,"beta_VAE_weights_HJ.pth")
...
with torch.no_grad():
    recon_tensor, mu, logvar = model(img_tensor)
...
plt.tight_layout() 
plt.savefig(os.path.join(output_dir,"reconstruction_beta_VAE_HJ.png"))
